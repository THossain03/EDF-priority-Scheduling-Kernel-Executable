...
int sleep_period_end_timestamps[MAX_TASKS];
...
void osKernelInit(void) {
	...
	total_thread_size = (uint32_t)MSP_INIT_VAL - MAIN_STACK_SIZE;
	...
	scheduler_TCB[0].stack_high = (char*)k_mem_alloc(STACK_SIZE);
	scheduler_TCB[0].stack_location = scheduler_TCB[0].stack_high + STACK_SIZE;
	scheduler_TCB[0].state = READY; //READY OR RUNNING FOR NULL TASK???
	scheduler_TCB[0].stack_size = STACK_SIZE;
	scheduler_TCB[0].deadline = MAX_TIMESTAMP; //Current deadline of each task set to 5ms
	scheduler_TCB[0].timestamp = MAX_TIMESTAMP;
	scheduler_TCB[0].sleep_count = 0; //-1
	...
	for (int i = 1; i < MAX_TASKS; i++) {
		...
		scheduler_TCB[i].deadline = 0; //Current deadline of each task set to 5ms
		scheduler_TCB[i].timestamp = 0;
		scheduler_TCB[i].sleep_count = 0; //-1
	}
...
int osKernelStart(void) { //Doesn't need to return RTX_OK, only RTX_ERR if anything goes wrong
	if (kernel_status == 1 || kernel_init == 0) { //If kernel not initialized or already running??
		...
		//assign a scheduler to find the tasks to run next in sequentially through the array.
		current_task = osScheduler();
		scheduler_TCB[current_task].state == RUNNING;
...
int osCreateTask(TCB* task) {
	...
	if (task->stack_size < STACK_SIZE || tot_task_size > MAX_TASKS) {
		return RTX_ERR;
	}

	for (int i = 1; i < MAX_TASKS; i++) {
		if (scheduler_TCB[i].state == DORMANT && scheduler_TCB[i].stack_size == 0x0) { //First time creating task in thread stack slot


				METADATA* temp = scheduler_TCB[i].stack_high - META_SIZE; //GET LOCATION OF ALLOCATED MEMORY BLOCK
			...
			scheduler_TCB[i].stack_high = (int*)k_mem_alloc(task->stack_size);
			/*
			 * CHECKING IF WE GOT SPACE TO CREATE THREAD STACK ON HEAP AND MODIFY OWNER OF MEMORY ALLOCATED----------------------------
			 */
				if (scheduler_TCB[i].stack_high == NULL) {
					return RTX_ERR;
				}

			scheduler_TCB[i].stack_location = (char*)scheduler_TCB[i].stack_high + task->stack_size;
			scheduler_TCB[i].state = READY;
			scheduler_TCB[i].deadline = 5; //Set to a default of 5ms
			scheduler_TCB[i].sleep_count = 0; //-1
			...
			*(--scheduler_TCB[i].stack_location) = (uint32_t)scheduler_TCB[i].ptask; //the function name
			...
			if (scheduler_TCB[i].timestamp < scheduler_TCB[current_task].timestamp && current_task != 0 && kernel_status) { //IF NEW TASK HAS SHORTER DEADLINE
...
int osTaskExit(void) {
	if (tot_task_size){ //If called by running tasks
		...
		k_mem_dealloc(scheduler_TCB[current_task].stack_high);
...
/*_____________________________________________________________________________________________________
 	 	 	 	 	 	 	 	 	 	 KERNEL PRE-EMPTIVE API FUNCTIONALITIES
_____________________________________________________________________________________________________*/
...
void osPeriodYield(void) {
	unsigned int required_sleep_time = scheduler_TCB[current_task].timestamp;
...
int osSetDeadline(int deadline, int TID) {
	...
	//SET DEADLINE OF TASK IN TCB
	if (TID > 0 && TID < MAX_TASKS && deadline > 0) { //CHECK FOR VALID TID FIRST...
		if (scheduler_TCB[TID].stack_high > 0x0 && scheduler_TCB[TID].state == READY) { //TASK IS PRESENT, NOT SURE TO CHECK IF TASK IS RUNNING OR NOT???
	...
	if (deadline < scheduler_TCB[current_task].timestamp) { //IF NEW TASK HAS SHORTER DEADLINE
...
int osCreateDeadlineTask(int deadline, int s_size, TCB* task) {
	...
	if (deadline <= 0 || s_size < STACK_SIZE || s_size > (HEAP_END - HEAP_START - META_SIZE) || tot_task_size > MAX_TASKS) {
		return RTX_ERR;
	}
	...
	for (int i = 1; i < MAX_TASKS; i++) {
		if (scheduler_TCB[i].state == DORMANT && scheduler_TCB[i].stack_size == 0x0) { //First time creating task in thread stack slot
			...
			scheduler_TCB[i].stack_high = k_mem_alloc(s_size);
			/*
			 * CHECKING IF WE GOT SPACE TO CREATE THREAD STACK ON HEAP AND MODIFY OWNER OF MEMORY ALLOCATED----------------------------
			 */
				if (scheduler_TCB[i].stack_high == NULL) {
				...
				METADATA* temp = (METADATA*)((char*)scheduler_TCB[i].stack_high - META_SIZE); //GET LOCATION OF ALLOCATED MEMORY BLOCK
				temp->owned_task = i; //SET NEW OWNER TO THE NEXT TASK BEING CREATED. THIS IS LEGAL SINCE WE'RE IN THE KERNEL -> NO VIOLATION OF SECURITY
			/*
			 * ------------------------------------------------------------------------------------
			 */
			...
			scheduler_TCB[i].stack_location = (char*)scheduler_TCB[i].stack_high + s_size;
			scheduler_TCB[i].state = READY;
			scheduler_TCB[i].deadline = deadline;
			scheduler_TCB[i].timestamp = deadline;
			scheduler_TCB[i].sleep_count = 0; //-1
			...
			*(--scheduler_TCB[i].stack_location) = (uint32_t)scheduler_TCB[i].ptask; //the function name
			...
			if (scheduler_TCB[i].timestamp < scheduler_TCB[current_task].timestamp && kernel_status) { //IF NEW TASK HAS SHORTER DEADLINE
